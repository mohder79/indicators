...
def EMA(data: str, column: str, length: int):
    return data[column].ewm(span=length, min_periods=length, adjust=False).mean()
# }


# { function for calculate EMA metod 2
def EMA2(data, src, Length: int):
    size = np.size(data[src]) - Length + 1
    EMA = np.zeros(np.size(data[src]))
    EMA[0] = np.mean(data[src][:Length])
    for i in range(1, size):
        EMA[i] = EMA[i-1] + (2/(Length + 1)) * \
            ((data[src][i+Length-1]) - EMA[i-1])
    return EMA
# }
